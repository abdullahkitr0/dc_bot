...
module.exports = {
  info: {
    ...
    usage: "",
    aliases: ["q", "list", "songlist", "song-list","queue","sıra"],
  },
  ...
  const permissions = message.channel.permissionsFor(message.client.user);
    if (!permissions.has(["MANAGE_MESSAGES", "ADD_REACTIONS"]))
      return sendError("Mesajları yönetmek veya reaksiyon eklemek için izin eksik",message.channel);
...
function generateQueueEmbed(message, queue) {
  ...
  for (let i = 0; i < queue.length; i += 10) {
    ...
    .setColor("BLUE")
    .setDescription(`${info}`)
    .addField("Şuan Çalan", `[${queue[0].title}](${queue[0].url})`, true)
    .addField("Yazı Kanalı", serverQueue.textChannel, true)
    .addField("Konuşma Kanalı", serverQueue.voiceChannel, true)
    .setFooter("Şu anda Sunucu Ses Seviyesi"+serverQueue.volume)
